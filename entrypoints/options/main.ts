...
form?.addEventListener('submit', async (event) => {
  ...
  setStatus('Validating Gemini API key and saving settings...', 'info');
...
resetButton?.addEventListener('click', async () => {
  if (!confirm('Reset the muted-image library? This cannot be undone.')) {
    return;
  }
...
clearCacheButton?.addEventListener('click', async () => {
  if (!confirm('Clear the scan cache? Future tweets will need new Gemini calls.')) {
    return;
  }
